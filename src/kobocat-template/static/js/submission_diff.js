...
function createTable(canEdit)
{
         ...
         if( (GROUP_TYPE === "repeat" && !GROUP_CREATED)|| (GROUP_TYPE === "group" && !GROUP_CREATED)){
                ...
                for(var i=0;i<Groups.length;i++){
                    if( groupNumber!=Groups[i].group_no && GROUP_TYPE!="group"){
                        var trGroupHeadData = $('<tr style="width:100%;color: #fff; background: black;"></tr>');
                        var tdGroupHeadData = $('<td style="align:center;color: #fff; background: black;" colspan="2">Group Data '+Groups[i].group_no+'</td>');
                        trGroupHeadData.append(tdGroupHeadData);
                        tBody.append(trGroupHeadData);
                        groupNumber = Groups[i].group_no;     
                    }
                    var tdGroupLabel = $('<td></td>');
                    tdGroupLabel.append(Groups[i].question);
                    var trgroupData = $('<tr class=""></tr>');
                    trgroupData.append(tdGroupLabel);
                    var tdgroupData = $('<td></td>');
    ...
    table.append(tHead);
    table.append(tBody);
    var textPrev = $('<div style="width:100%;background-color:rgb(29,174,227);padding:3px;"><span class="prev"><b>PREVIOUS</b></span></div>');
    dataContainer.append(textPrev);
...
function createNewTable(canEdit)
{
    ...
    table.append(tHead);
    table.append(tBody);
    var textAfter = $('<div style="width:100%;background-color:rgb(29,174,227);padding:3px;"><span class="after"><b>AFTER</b></span></div>');
    dataContainer.append(textAfter);
    dataContainer.append(table);
