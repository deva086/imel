...
def create_instance(username, xml_file, media_files,
    ...
    try:
        instance = None
        #print "logger tools ########-1"
        user = get_object_or_404(User, username=username.lower())
        #print user
        #print "logger tools ########-2"
        submitted_by = request.user \
            if request and request.user.is_authenticated() else user
        #print "logger tools ########-3"
        #print submitted_by
        if username:
            username = username.lower()
        #print "logger tools ########-3-1"

        xml = xml_file.read()
        #print "logger tools ########-3-2"
        #print username
        #print uuid
        #print xml
        xform = get_xform_from_submission(xml, username, uuid)
        #print "logger tools ########-3-3"
        check_submission_permissions(request, xform)
        #print "logger tools ########-4"
