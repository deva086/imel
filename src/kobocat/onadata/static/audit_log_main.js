...
function createTableRow(jsondata){
    ...
    for (i = 0; i < length; i++){
        var serial = i+1;
        var form_owner = jsonObj.instances[i].form_owner;
        ...
        var spTableRowDiffButton = $('<td class="tg-yw4l" align="center" style="width:90px;">'+
            '<button class="btn" type="button" onclick="gotoDiff(\''+form_id_string+'\',\''+form_owner+'\',\''+instanceID+'\',\''+data_id+'\')">View Diff</button></td>');
...
function gotoMerge(){
     window.open(user_id+'/forms/merge/did/');
       // window.location=user_id+'/forms/'+form_id_string+'/difference/'+instance_id;
    }