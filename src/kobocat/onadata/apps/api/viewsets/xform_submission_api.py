...
"""
    ...
    def create(self, request, *args, **kwargs):
        ...
        #print username
        if self.request.user.is_anonymous():
            #print "Submission fail #######################################-6-1"
            if username is None and password is None:
                # raises a permission denied exception, forces authentication
                self.permission_denied(self.request)
            else:
                #print "Submission fail #######################################-6-2"
                user = get_object_or_404(User, username=username.lower())
        ...
        error, instance = (create_instance_from_json if is_json_request else
                           create_instance_from_xml)(username, request)
        #print "Submission fail #######################################-8"

        if error or not instance:
            return self.error_response(error, is_json_request, request)

        print(user.has_perm('report_xform', instance.xform))
        if not user.has_perm('report_xform', instance.xform):
            return Response('User has not submission permission for this form',
                            headers=self.get_openrosa_headers(request),
