        ...
        if(dataGrid==null || dataGrid=='undefined')
                ...
                this.dataGrid = new Backgrid.Grid({
                    ...
                    columns: this.form.fields.map(function (f) {
                        //========================== start:added mpower ==================================
                        if(json_flag){
                            json_data = json_data + JSON.stringify(f);
                            json_flag = false ;
                        }else{
                            json_data =  json_data + "," +JSON.stringify(f) ;
                        }
                ...
                var allowed_types = ["select one","select all that apply","text","decimal","integer"];
                for(var i in json_data_array){
                      if(contains(allowed_types, json_data_array[i].type)){
                        filtered_data.push(json_data_array[i]);
                    }
                }
//                console.log(JSON.stringify(filtered_data));
               $.each(filtered_data, function(i,obj)
                {
                    $(".criteria").append(
                         $('<option></option>')
                                .val(i)
                                .html(filtered_data[i].name));
                    if (json_data_array[i].type === 'select one' ) {
                        
                        if(filtered_data[i].label !== null && typeof filtered_data[i].label === 'object'){
                            $(".audio_filters").append(
                             $('<option></option>')
                                    .val(i)
                                    .html(filtered_data[i].label.default));
                        }else{
                            $(".audio_filters").append(
                             $('<option></option>')
                                    .val(i)
                                    .html(filtered_data[i].label));
                        }
                    }
                });
        ...
        filter.$el.css({float: "right", margin: "25px -180px 0px 0px"});
                // Render the filter
...
//      $( "input[id='toDate']" ).val(todate);
        ...
        searchButtonTest.$el.css({float: "right", margin: "70px"});
        if ($('.filters').length){
            $('.filters').append(searchButtonTest.render().$el);
        }else{
            this.$el.prepend(searchButtonTest.render().$el);
                ...
                this.data.fetch({reset: true}).promise().done(function(data){
                    // loops through each data submission
...
function getQuery(){
    ...
    $('#filter-table tr td:nth-child(2)').each(function(){
        ...
        if(count == 0){
            ...
            var selected_user = $( "#userlist" ).val() || [];
            if (selected_user.length > 0) {
                var q_string = '[';
                selected_user.forEach(function(user){
                    q_string += '"' + user + '" ,'
                })
                q_string = q_string.substring(0, (q_string.length -1 )) + ']';
                var partial_query = '{ "$or" : [{ "_submitted_by": { "$in" : ' + q_string + ' } }' ;
                partial_query += ' , { "username": { "$in" : ' + q_string + ' } } ] }';
                query += partial_query + ',' ;
            // }
            // if ( selected_user !== 'custom') {
            //     // {"_submitted_by":{ "$in" : ["ratna","himel"] }
...
function get_label(attachment_name,data) {
    var form_label = 'Label not available';
    data.forEach(function(object) {
        for (var property in object) {
            if (object.hasOwnProperty(property) && object[property]===attachment_name) {
                // console.log(property + " : " + object[property]);
                json_data_array.forEach(function(submission) {  
                    // console.log(submission['name']);
                    if (property === submission['name']){
                        if(submission['label'] !== null && typeof submission['label'] === 'object'){
                            form_label = submission['label'].default;
                        }else{
                            form_label = submission['label'];
                        }
                    }
                });
            }
