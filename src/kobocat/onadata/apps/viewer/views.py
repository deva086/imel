...
from onadata.apps.usermodule.models import UserModuleProfile, OrganizationDataAccess, Organizations
import logging
...
def data_approve(request):
    ...
    logging.info('(apps/viewer/views.py) Action: Approving data...')
    ...
    approveLine = None
    try:
        approval = Approval.objects.filter(formid=formid,subbmissionid=submissionid,status="Pending").distinct()
	for value in approval:
	    print(value.userid)	
            value.status = "Approved"
            value.save(update_fields=['status'])
        approveLine = Approveline.objects.get(formid=formid,label = int(approval[0].label) )
    except Approveline.DoesNotExist:
        approveLine = None
    if approveLine is not None:
        approveObject = Approval()
        approveObject.formid = formid
        approveObject.subbmissionid = submissionid
        approveObject.status = "Pending"
        approveObject.userid = approveLine.userid
        approveObject.label = int(approveLine.label+1)
        approveObject.save()
        logging.info(approval.status)
        xform_instances.update({"_id": int(submissionid) },{'$set':{"approve":"processing"}})
    else:
        approvalCount = Approveline.objects.filter(formid=formid).count();
        approvalLineCount = Approval.objects.filter(formid=formid,subbmissionid=submissionid).count();

        if(approvalCount == approvalLineCount):
            xform_instances.update({"_id": int(submissionid) },{'$set':{"approve":"Approved"}})
        else:
			 approvalCount = None
    #xform_instance.remove(9)
    return HttpResponse("You're voting at poll %s.")

def data_reject(request):
    logging.basicConfig(filename='my.log',level=logging.DEBUG)
    logging.info('(apps/viewer/views.py) Action: Rejecting data...')
    submissionid = request.POST.get('submissionid')
    userid= request.user.username
    formid= request.POST.get('formid')
    approveLine = None
    ...
    if approveLine is not None:
        ...
        approveObject.save()
        logging.info(approval.status)
        xform_instances.update({"_id": int(submissionid) },{'$set':{"reject":"processing"}})
    else:
        approvalCount = Approveline.objects.filter(formid=formid).count();
        approvalLineCount = Approval.objects.filter(formid=formid,subbmissionid=submissionid).count();
        logging.info("kobbbbbbbbbb")
        logging.info(approvalLineCount)
        logging.info(approvalCount)
        if(approvalCount == approvalLineCount):
            xform_instances.update({"_id": int(submissionid) },{'$set':{"reject":"Reject"}})
        else:
			 approvalCount = None
...
def export_list(request, username, id_string, export_type):
    ...
    owner = get_object_or_404(User, username__iexact=username)
    xform = get_object_or_404(XForm, id_string__exact=id_string, user=owner)
    # users_submitted_by = get_users(request.user)
    users_submitted_by2 = get_users_by_orgs(request)
    # flag = has_form_username(xform.json)
    flag = has_form_username(xform.json)
    if not has_permission(xform, owner, request):
        return HttpResponseForbidden(_(u'Not shared.'))
...
#                _("%s is not a valid export type" % export_type))
    ...
    data = {
        ...
        'owner': owner, # mpower :: added later for dynamic filtering
        # 'users_submitted_by':users_submitted_by, # mpower :: added later for dynamic filtering 
        'users_submitted_by2':users_submitted_by2, # mpower :: added later for dynamic filtering 
        'xform': xform,
        'export_type': export_type,
...
def get_users(user):
    ...
    # users_submitted_by.append(str(user.username))
    if current_user:
        current_user = current_user[0]
        observables = OrganizationDataAccess.objects.filter(observer_organization=current_user.organisation_name)
        for o in observables:
            users_of_current_org = UserModuleProfile.objects.filter(organisation_name=o.observable_organization)
            for org_user in users_of_current_org:
                users_submitted_by.append(str(org_user.user.username))
        users_submitted_by = json.dumps(users_submitted_by)
    return users_submitted_by


def get_users_by_orgs(request):
    optgroups = []
    if request.user.is_superuser:
        ...
        for o in org_list:
            users_submitted_by = []
            optgroup_dict = {}
            optgroup_dict['label'] = o.organization
            users_of_current_org = UserModuleProfile.objects.filter(organisation_name=o)
            for org_user in users_of_current_org:
                option_dict = {}
                option_dict['label'] = str(org_user.user.username)
                option_dict['value'] = str(org_user.user.username)
                users_submitted_by.append(option_dict)
            optgroup_dict['children'] = users_submitted_by
            optgroups.append(optgroup_dict);
    else:            
        ...
        if current_user:
            ...
            for o in observables:
                users_submitted_by = []
                optgroup_dict = {}
                optgroup_dict['label'] = o.observable_organization.organization
                users_of_current_org = UserModuleProfile.objects.filter(organisation_name=o.observable_organization)
                for org_user in users_of_current_org:
                    option_dict = {}
                    option_dict['label'] = str(org_user.user.username)
                ...
                optgroup_dict['children'] = users_submitted_by
                optgroups.append(optgroup_dict);
    return json.dumps(optgroups)


def has_form_username(xjson):
...
def data_view(request, username, id_string):
    ...
    data = {
        'owner': owner,
        'xform': xform,
        # 'users_submitted_by': users_submitted_by,
        'users_submitted_by2': users_submitted_by2,
        # 'flag':flag
    }
    audit = {
        "xform": xform.id_string,
...
def instance_old(request, username, id_string):
    logging.basicConfig(filename='my.log',level=logging.DEBUG)
    logging.info('(apps/viewer/views.py) Action: data view from instance controller')
    xform, is_owner, can_edit, can_view = get_xform_and_perms(
        username, id_string, request)
    ...
    logging.info('ss_id = '+ _id_string)
    approvals = Approval.objects.filter(userid=request.user.username,formid=id_string,subbmissionid=_id_string).first()
    is_approved_or_reject = 'true'
    if approvals is not None:
       is_approved_or_reject = 'false'
       if approvals.status == 'Approved' or approvals.status == 'Reject':
          is_approved_or_reject = 'true'
          logging.info('(apps/viewer/views.py) is_approved_or_reject = ' + is_approved_or_reject)
