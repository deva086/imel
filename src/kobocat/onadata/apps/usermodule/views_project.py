...
from onadata.apps.viewer.views import get_users,has_form_username
...
def test(request, username, id_string):
    ...
    return render(request, "usermodule/test.html", data)


def get_ajax_response(request):
    key1 = request.GET.get('key1')
    key2 = request.GET.get('key2')
    mongo_results_all = xform_instances.aggregate([ { "$match":  {
                                                                key1: {"$ne":None}, # None because pymongo, in mongo null should be used
                                                                key2: {"$ne":None}  # None because pymongo, in mongo null should be used
                                                                } 
                                                    },
                                                    { 
                                                    "$group":   {
    ...
    output = json.dumps(dictmongo(mongo_results_all))
    return HttpResponse(output,content_type='application/json'); 
...
def dictmongo(cursor):
    ...
    for key in cursor['result']:
        keyVal = {}
        keyVal['id'] = key['_id']
        keyVal['count'] = key['count']
        # if bool(key['_id']):
        value_list.append(keyVal)
    return value_list